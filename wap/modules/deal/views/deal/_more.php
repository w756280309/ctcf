    ...
    <a class="row col" href="/deal/deal/detail?sn=<?= $val->sn ?>">
        ...
        <div class="col-xs-12 col-sm-12 col-txt">
            <div class="row clearfix credit-num">
                ...
                <div class="col-xs-2 col-sm-2 col-title"><i class="credit-staus <?= !in_array($val->status, [1, 2]) || $val->end_date < time() ? 'credit-staus-over' : '' ?>"><?= ($val->end_date < time() && in_array($val->status, [1, 2])) ? '募集结束' :Yii::$app->params['deal_status'][$val->status] ?></i></div>
            </div>
