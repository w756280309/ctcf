<?php
/* @var $this yii\web\View */

$this->title = 'Show Main';
use common\models\AuthSys;
$menus = AuthSys::getMenus('A1000000');
?>
<?php $this->beginBlock('block1eft'); ?>

<td width="180" id="page_left" height="100%" align="left" valign="top">
    <!--左边-->
    <div id="nav" class="scroll-pane" style="height: 591px;"><div class="title"><a href="javascript:void(0)">内容管理首页</a></div>
        <ul class="load menu">
            <?php foreach ($menus as $val){ ?>
                <li><a href="/<?=$val['path']?>" target="main"><?=$val['auth_name']?></a></li>
            <?php } ?>
<!--            <li><a href="/adv/pos/edit" target="main">添加广告位</a></li>
            <li><a href="/adv/pos/index" target="main">广告位列表</a></li>
            <li><a href="/adv/adv/edit" target="main">添加广告</a></li>
            <li><a href="/adv/adv/index" target="main">广告列表</a></li>-->
        </ul>
    </div>
</td>
<?php $this->endBlock(); ?>

<?php $this->beginBlock('blockmain'); ?>
<td height="100%" style="width:100%\9" align="left" valign="top">
    <!--右边-->
    <div style="position:relative; width:100%; height:100%;">
        <div class="loading" id="content_loading" style="display: none;"></div>
        <iframe id="main" name="main" src="/site/index" frameborder="0"></iframe>

    </div>
</td>
<?php $this->endBlock(); ?>

