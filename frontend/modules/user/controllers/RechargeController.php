...
class RechargeController extends BaseController
{
     ...
     * 借款账户充值
     * */
    public function actionBorrowerRecharge(){
        $this->layout = 'main';
        Yii::$app->session->set('to_url', '/user/recharge/borrower-recharge');
        ...
        $recharge = new RechargeRecord();
        $user_account = $this->user->borrowAccount;
        ...
        return $this->render('borrowerrecharge', [
            'recharge' => $recharge,
            'user_account' => $user_account,
            'bank' => $bank,
            'url' => $url,
    ...
    public function actionApply()
    {
         ...
         * 借款用户充值表单提交一面
         */
    public function actionBorrowerApply()
    {
        //检查是否开户
        $cond = 0 | BankService::IDCARDRZ_VALIDATE_N;
        $data = BankService::check($this->user, $cond);
        ...
        $recharge = new RechargeRecord([
            ...
            'pay_type' => $pay_type,
            'pay_id' => 0,
            'account_id' => $this->user->borrowAccount->id,
            'uid' => $this->user->id,
            'bank_id' => $bank_id, //网银充值,不要求绑卡,存储的是银行的统一ID
            'pay_bank_id' => $bank_id, //网银充值,不要求绑卡,存储的是银行的统一ID
            'status' => RechargeRecord::STATUS_NO,
        ...
        if ($recharge->load(Yii::$app->request->post()) && $recharge->validate()) {
            ...
            $ump->borrowerRechargeViaBpay($recharge, $bank->bank->gateId);
        } else {
            return $this->redirect('/info/fail?source=chongzhi&jumpUrl=/user/recharge/init');
        }
    }
