...
class RopUtils{
	...
	static function sign($paramValues,$ignoreParamNames,$headerMap,$extInfoMap){
		$secret=\Yii::$app->params['bao_quan_config']['app_secret'];
	...
	static function getHeads($v,$method){
		...
		$headerMap=array(
				...
				"v"=>$v,
				"method"=>$method,
				"appKey"=>\Yii::$app->params['bao_quan_config']['app_key']);
		return $headerMap;
	}
	...
	static function doPost($paramValues,$ignoreParamNames,$headerMap,$contentType="application/x-www-form-urlencoded; charset=UTF-8"){
		...
		$response = file_get_contents(\Yii::$app->params['bao_quan_config']['services_url'], false, $request);
		return $response;
	}
