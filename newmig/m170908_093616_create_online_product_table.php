...
class m170908_093616_create_online_product_table extends Migration
{
    ...
    public function up()
    {
        $this->createTable('online_product', [
            ...
            'issuer'=>$this->integer(11)->defaultValue(Null),
            'issuerSn'=>$this->string(30)->defaultValue(Null),
            'paymentDay'=>$this->integer(5)->defaultValue(Null),
            'isTest'=>$this->boolean()->defaultValue(0),
            'filingAmount'=>$this->decimal(14,2)->defaultValue(Null),
            'allowUseCoupon'=>$this->boolean()->notNull(),
            'tags'=>$this->string(255)->defaultValue(Null),
            'isLicai'=>$this->boolean()->defaultValue(Null),
            'pointsMultiple'=>$this->smallInteger(6)->defaultValue(1),
            'allowTransfer'=>$this->boolean()->defaultValue(1),
            'isCustomRepayment'=>$this->boolean()->defaultValue(Null)->comment('是否自定义还款'),
            'isJixiExamined'=>$this->boolean()->defaultValue(1)->comment('计息审核'),
            'internalTitle'=>$this->string(255)->defaultValue(Null),
            'publishTime'=>$this->dateTime()->defaultValue(Null),
            'balance_limit'=>$this->decimal(14,2)->defaultValue(0.00),
        ]);
    }
