...
class BankBackNotifyAction extends Action
{
    public function run()
    {
        ...
        Yii::info('绑卡后台通知 ump_log user_bank_back_notify user_id: ' . $user->id . ';cardNo:.' . $bind->card_number . ';mobile:' . $user->mobile . '; ret_code:' . $data['ret_code'] . ';ret_msg:' . $data['ret_msg'], 'umplog');
        if (
