...
class PayService {
    ...
    public function checkAllowPay($sn=null,$money=null,$tradepwd = null){
        ...
        if(bcdiv($orderbalance,$this->cdeal->start_money)*1>=1){//若可投金额大于起投金额
            ...
            }else if(bcdiv($orderbalance,$money)*1<1){ //可投金额除以投标金额，如果是小于1的数字，代表超额投资
                return ['code'=>self::ERROR_MONEY_MUCH,  'message'=>self::getErrorByCode(self::ERROR_MONEY_MUCH)];
            }else if($this->cdeal->dizeng_money/1){
                bcscale(14);
