...
class RSA
{
     ...
     * @return string 签名
     */
    public static function sign($data, $privateKeyFilePath, $type=OPENSSL_ALGO_SHA1)
    {
     ...
     * @return bool 验签是否成功
     */
    public static function verify($data, $sign, $rsaPublicKeyFilePath, $type=OPENSSL_ALGO_SHA1)
    {
    ...
    public static function rsaEncrypt($data, $pubKeyFilePath)
    {
        ...
        //转换为openssl格式密钥
        $res = openssl_get_publickey($pubKey);
        $maxlength = RSA::getMaxEncryptBlockSize($res);
        $output = '';
        while ($data) {
    ...
    public static function rsaDecrypt($data, $privateKeyFilePath)
    {
        ...
        $maxlength = RSA::getMaxDecryptBlockSize($res);
        $output = '';
        while ($data) {
            $input = substr($data, 0, $maxlength);
            $data = substr($data, $maxlength);
