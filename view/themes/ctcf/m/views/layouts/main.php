...
<div class="container">
    <?php if (!defined('IN_APP') && !$this->hideHeaderNav) { ?>
        <?php if ($this->headerNavOn) { ?>
            <header class="row head-title">
                <div class="logo col-xs-12 col-sm-12"><img src="<?= ASSETS_BASE_URI ?>images/ctcf/logo.png" alt="logo"></div>
    ...
    <div class="nav-box <?= (!defined('IN_APP') && $this->showBottomNav) ? '' : 'no-margin-bottom' ?>">
        ...
        <p>客服热线：<a class="contact-tel" href="tel:<?= Yii::$app->params['platform_info.contact_tel'] ?>"><?= Yii::$app->params['platform_info.contact_tel'] ?></a>（<?= Yii::$app->params['platform_info.customer_service_time'] ?>）</p>
    </div>
    <?php } ?>
