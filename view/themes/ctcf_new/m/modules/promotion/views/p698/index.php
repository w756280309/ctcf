...
<script>
    $(function () {
        ...
        var c2 = "<?= $flags['c2'] ?>";
        if(!!c1){
            $("#btn-one").removeClass("btn-red").addClass("btn-grey").html("已领180元红包").unbind("click");
        }
        if(!!c2){
            $("#btn-two").removeClass("btn-red").addClass("btn-grey").html("已领230元红包").unbind("click");
        }
        ...
        function openPopup(index) {
            ...
            if (index == 1) {
                type = "c1";
                message = '代金券在领取前会一直保留，一旦领取将进入有效期，建议您需要使用的时候再领取。<br><br>是否现在就领取180元代金券（100万起借，有效期90天）？';
            } else {
                type = "c2";
                message = '代金券在领取前会一直保留，一旦领取将进入有效期，建议您需要使用的时候再领取。<br><br>是否现在就领取230元代金券（200万起借，有效期90天）？';
            }
            layer.open({
                title: [
                    '温馨提示',
