<?php $uid = null !== $user ? $user->getId():0;?>
<div class="piD-data-box">
    <?php if(empty($uid)){?>
        <p class="not_yet">【<a onclick="login()" style="cursor: pointer;color: #419bf9;">登录</a>】后才能查看</p>
    <?php }else{?>
        <?php if(count($data)>0){?>
            <table class="piD-data-inner">
                ...
                <?php foreach($data as $k=>$v) { ?>
                    <tr style="<?= ($k%2 != 0) ? "background: rgb(239, 239, 243);":"" ?>">
                        ...
                        <td class="piD-th1 piD-left"><span><?= $v->user ? \common\utils\StringUtils::obfsMobileNumber($v->user->mobile) : "---" ?></span></td>
